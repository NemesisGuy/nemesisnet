...
The JavaScript code in `index.html` is responsible for the following:

- **Theme Switching**: The `setTheme`, `toggleTheme`, `setNemesis`, `toggleNemesisMode`, `setAurora`, and `toggleAuroraMode` functions manage the site's theme.
- **Navigation**: The `openNav`, `closeNav`, and `toggleNav` functions control the mobile navigation menu.
- **Animations**: The `type` function creates a typing animation in the header.
- **Form Handling**: The `handleContact` function handles the contact form submission.
- **Footer**: The `updateFooter` function updates the footer with the current year and month.
- **Scroll Behavior**: The code also includes functionality for fade-in scroll animations and active state highlighting for navigation links.
