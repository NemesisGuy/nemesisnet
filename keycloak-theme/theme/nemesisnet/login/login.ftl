...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
        Sign in to NemesisNet
    <#elseif section = "form">
        <div id="kc-form">
          <div id="kc-form-wrapper">
            <#if realm.password>
                <div class="nn-logo-wrapper">
                    <img src="${url.resourcesPath}/img/logo.png" alt="NemesisNet" class="nn-logo" />
                </div>
                <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    <#if !usernameHidden??>
                        <div class="nn-input-group">
                            ...
                            <#if messagesPerField.existsError('username','password')>
                                <span class="nn-field-error" aria-live="polite">
                    ...
                    <div class="nn-checkbox-row">
                        <#if realm.rememberMe && !usernameHidden??>
                            <label class="nn-checkbox-label">
                        ...
                        <#if realm.resetPasswordAllowed>
                            <a tabindex="6" class="nn-link" href="${url.loginResetCredentialsUrl}">Forgot password?</a>
                        </#if>
                    </div>
                    ...
                    <div id="kc-form-buttons">
                        ...
                        <input tabindex="7" class="nn-btn nn-btn-primary" name="login" id="kc-login" type="submit" value="Sign In"/>
                    </div>
                </form>
            </#if>
          </div>
        ...
        <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
            <div id="kc-registration-container">
                <div id="kc-registration">
                    <span class="nn-info" style="text-align: center; display: block;">
                        Don't have an account? <a tabindex="8" class="nn-link" href="${url.registrationUrl}">Register</a>
                    </span>
                </div>
            </div>
        </#if>
    <#elseif section = "socialProviders">
        <#if realm.password && social?? && social.providers?has_content>
            <div class="nn-divider"><span>or continue with</span></div>
            <div id="kc-social-providers" class="nn-social-providers">
                <#list social.providers as p>
                    <a id="social-${p.alias}" class="nn-social-btn" type="button" href="${p.loginUrl}">
                        <#if p.alias == 'github'>
