import { defineEventHandler, getRequestHeader, createError, setResponseHeader, readBody } from 'file://C:/Users/Reign/Projects/NemesisNet/node_modules/h3/dist/index.mjs';
...
function checkRateLimit(ip) {
  ...
  if (entry.tokens <= 0) {
    ...
    return { allowed: false, retryAfter: Math.ceil(waitMs / 1e3) };
  }
  entry.tokens--;
  return { allowed: true };
}
...
const chat_post = defineEventHandler(async (event) => {
  ...
  const sanitizedHistory = Array.isArray(history) ? history.slice(-10).filter((msg) => msg.role === "user" || msg.role === "model").map((msg) => ({
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (gemmaRes.ok) {
        ...
        if (text) return { text };
        const finishReason = (_e = (_d = data.candidates) == null ? void 0 : _d[0]) == null ? void 0 : _e.finishReason;
        if (finishReason === "SAFETY" || !((_f = data.candidates) == null ? void 0 : _f.length)) {
