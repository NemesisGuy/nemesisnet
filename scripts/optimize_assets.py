...
# Output tuning
PROJECT_TARGET_WIDTH = 900
BRAND_FULL_WIDTH = 256
BRAND_NAV_WIDTH = 96
TARGET_DPI = (72, 72)
WEBP_QUALITY = 88
...
def discover_project_pairs() -> Iterable[Pair]:
    ...
    for project_dir in PROJECTS_DIR.iterdir():
        ...
        for source in original_dir.iterdir():
            ...
            yield source, destination, PROJECT_TARGET_WIDTH


def brand_pairs() -> Iterable[Pair]:
    """Return pairs for brand imagery (logo/favicon)."""

    brand_png = BRAND_DIR / "Nemesis_Logo_Icon.png"
    if brand_png.exists():
        yield brand_png, BRAND_DIR / "Nemesis_Logo_Icon.webp", BRAND_FULL_WIDTH
        yield brand_png, BRAND_DIR / "Nemesis_Logo_Icon_nav.webp", BRAND_NAV_WIDTH


def optimize_image(source: Path, destination: Path, max_width: int) -> None:
...
def main() -> None:
    ...
    for source, destination, max_width in pairs:
        optimize_image(source, destination, max_width)
